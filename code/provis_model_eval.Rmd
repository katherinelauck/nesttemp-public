---
title: "Provisioning models evaluation"
output:
  html_document:
    df_print: paged
---
...
gtsave(tab,"../figures/int_tab_provis_webl.html")
...
```{r}
(t <- emtrends(m_linint,specs = ~ habitat, var = c("mean_temp_scaled"),max.degree = 1) %>% test() %>%
...
gtsave(t,"../figures/weblprovistrend.html")
...
```{r}
(t <- emmeans(m_linint,"habitat") %>% regrid() %>% pairs() %>% as_tibble() %>% 
   mutate(across(estimate:z.ratio,~round(.x,digits = 2)),
          across(p.value,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/provisbyhabitat_webl.html")
```

### Check for effect of temperature

```{r}
(t <- summary(m_noint) %>% coef() %>% pluck("cond") %>% as_tibble(rownames = "Covariate") %>%
...
gtsave(t,"../figures/provisbyhabitat_summary_webl.html")
```
...
```{r}
m <- glmmTMB(valid_detections ~ poly(mean_temp_scaled,degree = 2) * habitat + julian_date_scaled + poly(tod_h_scaled,2) + mean_nestling_age_scaled + (1|attempt_id) + year_fct,
						 ziformula = ~ 1,
						 family = nbinom2(),
					data = dplyr::filter(p,!is.na(mean_temp),
					              !is.na(julian_date),
...
m_linint <- glmmTMB(valid_detections ~ mean_temp_scaled * habitat + poly(mean_temp_scaled,degree = 2) + julian_date_scaled + poly(tod_h_scaled,2) + mean_nestling_age_scaled + (1|attempt_id) + year_fct,
						 ziformula = ~ 1,
						 family = nbinom2(),
					data = dplyr::filter(p,!is.na(mean_temp),
					              !is.na(julian_date),
...
m_noint <- glmmTMB(valid_detections ~ poly(mean_temp_scaled,degree = 2) + habitat + julian_date_scaled + poly(tod_h_scaled,2) + mean_nestling_age_scaled + (1|attempt_id) + year_fct,
						 ziformula = ~ 1,
						 family = nbinom2(),
					data = dplyr::filter(p,!is.na(mean_temp),
					              !is.na(julian_date),
...
(tab <- c %>%
  ...
  mutate(across(c(AIC,Chisq), ~ round(.x, digits = 2)),
         P = if_else(P < 0.001,"<0.001",as.character(P))) %>% gt())
gtsave(tab,"../figures/int_tab_provis_tres.html")
...
ggsave("../figures/fig5_abscort_by_temp_hab.png",p_full,width = 6.25,height = 4)
```

## Emmeans to check for effect of habitat

```{r}
(t <- emmeans(m,"habitat") %>% regrid() %>% pairs() %>% as_tibble() %>% 
   mutate(across(estimate:z.ratio,~round(.x,digits = 2)),
          across(p.value,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/provisbyhabitat_tres.html")
```
...
(t <- summary(m_noint) %>% coef() %>% pluck("cond") %>% as_tibble(rownames = "Covariate") %>%
   ...
   mutate(across(Estimate:`z value`,~round(.x,digits = 2)),
          across(`Pr(>|z|)`,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/provisbyhabitat_summary_tres.html")
```


```{r}
(t <- emtrends(m,specs = ~ degree | habitat, var = "mean_temp_scaled",max.degree = 2) %>% test() %>%
   mutate(across(where(is.numeric), ~ round(.x, digits = 3)),
          df = round(df),
          p.value = if_else(p.value == 0.000,"<0.001",as.character(p.value))) %>%
   rename(Habitat = "habitat", `Max temp trend` = "mean_temp_scaled.trend",Df = "df", `Z-ratio` = "z.ratio", P = "p.value") %>%
...
gtsave(t,"../figures/tresprovistrend.html")
