...
gtsave(tab,"../figures/int_tab_survival_webl.html")
...
c1 <- anova(s_nestpd_WEBL,s_nestpd_WEBL_addmin,s_nestpd_WEBL_noint) %>% tibble() %>% mutate(Model = c("no interaction","single interaction","both interacting"),.before = Df)

c2 <- anova(s_nestpd_WEBL,s_nestpd_WEBL_addmax,s_nestpd_WEBL_noint) %>% tibble() %>% mutate(Model = c("no interaction","single interaction","both interacting"),.before = Df)
...
(tab <- bind_rows(c1,c2) %>%
  ...
  mutate(max_or_min = c(rep("Max temp",times = 3),rep("Min temp",times = 3)),
         ...
         P = if_else(P < 0.001,"<0.001",as.character(P))) %>%
  group_by(max_or_min) %>% gt())
gtsave(tab,"../figures/int_tab_survival_nestpd_webl.html")
...
(t <- summary(s_nestpd_WEBL_addmin) %>% coef() %>% as_tibble(rownames = "Covariate") %>%
   # dplyr::filter(Covariate != "poly(mean_temp_scaled, 2)1") %>%
   mutate(across(Estimate:`t value`,~round(.x,digits = 2)),
          across(`Pr(>|t|)`,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/survivalbyhabitat_summary_webl.html")
```
...
```{r}
(t <- emtrends(s_nestpd_WEBL_addmin,specs = pairwise ~ habitat, var = c("meanmaxt_nestpd_scaled")) %>% test() %>% pluck("emtrends") %>%
   mutate(across(where(is.numeric), ~ round(.x, digits = 3)),
          df = round(df),
          p.value = if_else(p.value == 0.000,"<0.001",as.character(p.value))) %>%
...
gtsave(t,"../figures/weblsurvival_nestpd_trendmax.html")
...
```{r}
(t <- emmeans(s_nestpd_WEBL_addmin,"habitat") %>% pairs() %>% as_tibble() %>%
   mutate(across(estimate:z.ratio,~round(.x,digits = 2)),
          across(p.value,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/survival_nestpd_byhabitat_webl.html")
```
...
# samp %>% gt() %>% gtsave("../figures/ss_webl.html")
# 
data_webl %>% group_by(habitat,year_fct) %>% summarize(count = n()) %>%
  pivot_wider(values_from = count,names_from = year_fct) %>% as.tibble() %>% rename(Habitat = 'habitat') %>% ungroup() %>% mutate(Total = `2021` + `2022` + `2023`) %>% gt() %>% 
...
c1 <- anova(s_nestpd_TRES,s_nestpd_TRES_addmin,s_nestpd_TRES_noint) %>% tibble() %>% mutate(Model = c("no interaction","single interaction","both interacting"),.before = Df)

c2 <- anova(s_nestpd_TRES,s_nestpd_TRES_addmax,s_nestpd_TRES_noint) %>% tibble() %>% mutate(Model = c("no interaction","single interaction","both interacting"),.before = Df)
...
(tab <- bind_rows(c1,c2) %>%
  ...
  mutate(max_or_min = c(rep("Max temp",times = 3),rep("Min temp",times = 3)),
         ...
         P = if_else(P < 0.001,"<0.001",as.character(P))) %>%
  group_by(max_or_min) %>% gt())
gtsave(tab,"../figures/int_tab_survival_nestpd_tres.html")
...
```{r}
(t <- emmeans(s_nestpd_TRES_noint,"habitat") %>% pairs() %>% as_tibble() %>%
   mutate(across(estimate:z.ratio,~round(.x,digits = 2)),
          across(p.value,~round(.x,digits = 3))) %>% gt())
gtsave(t,"../figures/survival_nestpd_byhabitat_tres.html")
```
...
# samp %>% gt() %>% gtsave("../figures/ss_TRES.html")
s_nestpd_TRES_noint$data %>% group_by(habitat,year_fct) %>% summarize(count = n()) %>%
  pivot_wider(values_from = count,names_from = year_fct) %>% as.tibble() %>% rename(Habitat = 'habitat') %>% ungroup() %>% mutate(Total = `2021` + `2022` + `2023`) %>% gt() %>% 
  grand_summary_rows(columns = -c(Habitat),fns = list(id = "Total") ~ sum(.)) %>%
