---
title: "Does adding temp * julian date interaction improve model fit for results?"
output:
...
1. Add interaction between julian date and max temp to model for each response: growth, survival, cort, provisioning
2. Determine whether that interaction improves the fit of the model
3. Determine whether adding that interaction changes the results (probably will need to make a table) 
4. Describe the direction of interaction for each response
...
For WEBL growth, adding the interaction improves fit.

Do the results change?

##### Comparing temp trend and significance across land covers without and with temp * julian date interaction
...
No direction or significance change, or really large coefficient change, between the two models.

##### Plot of growth without interaction
...
##### Plot of growth with interaction
...
##### Visualize temp * juliandate interaction
...
#### Conclusion: for WEBL growth, the interaction improves fit, but does not change growth results. In addition, the direction of interaction is opposite that expected: later in the season, hotter temps are worse for growth than in earlier season.
...
For TRES growth, same model is significant. Interaction is not significant. Adding the interaction does not improve fit.
...
(trend_comp <- tresgrowthtrendmax_prior %>% rbind(tresgrowthtrendmax) %>% gt())
...
#### Conclusion: for TRES growth, the interaction neither improves fit nor changes the results. There does not appear to be an interaction between juliandate and max temperature for TRES growth.
...
Interaction does not improve model fit for survival.
...
Forest trend becomes slightly more significant with the addition of the interaction

##### Plot of survival without interaction
...
##### Visualize direction of interaction
...
#### Conclusion: for WEBL survival, the interaction is not significant nor does it improve fit. Adding the interaction slightly changes p-values, which eliminates marginal significance in orchard and grassland, leaving only Forest with a marginally significant negative temp trend. It does not change the magnitude or direction of the effect of temperature.
...
Interaction improves model fit for survival - same model comes out of the comparison
...
Basically no change - temp trend becomes slightly more positive with addition of interaction but is still very small in magnitude and not significant.
...
#### Conclusion: for TRES survival, the interaction is improves fit but does not alter the overall findings re: survival. The interaction between temp and julian date is as expected: Earlier in the season, hot temps decrease survival more than hot temps later in the season.





### Baseline cort by prior week temp
...
Same conclusion here - noint is the most supported model, and the additon of the temp * julian date interaction doesn't improve fit.
...
Same results - very slightly positive and not significant. I don't think I'm even going to graph that.

##### Visualization of the temp * julian date interaction
...
Looks like baseline cort is higher later in the season, but there isn't much of an interaction.

##### Conclusion: juliandate * temp interaction neither adds to the fit nor changes the results.
...
Same model results, and temp * julian date does not add to model fit
...
##### Interaction is not significant, does not add to fit, does not change results.

### Abs difference cort by prior week temp
...
Does not add to fit, is not significant in model
...
No change to coefficients or significance
...
This is the first instance where including the interaction changes which model is the most parsimonious. Here, the interaction with max temp seems obviously more important vs before, the linear temp model was the chosen model. However, the interaction is significant in neither model, and improves fit neither for the model with both min and max interaction with land cover nor for just max interaction with land cover.
...
There are differences between the final outcome of these two analyses, but those are mostly because of the different structure of the most supported model. The direction of the estimated trends are the same.
...
Honestly the results here seem easier to explain.
...
Interaciton is not significant.

### s2 cort by prior week temp
...
Same model, interaction is not significant, does not add to fit
...
Same direction and significance of trends
...
Interaction is not significant, does not increase fit, does not change most parsimonious model
...
### Baseline cort by prior day temp
...
Interaction not significant, does not add to fit, does not change most parsimonious model
...
No change in trend or significance
...
So this more clearly favors addmax. I'm going to change the manuscript to reflect this.

Interaction is not significant, does not increase fit.
...
### Abs difference by prior day temp
...
Interaction not significant, does not add to fit, does not change parsimony
...
Interaction not significant, does not improve fit, does not change parsimony
...
Same trend and non significance
...
Not significant, does not improve fit, does not change parsimony
...
No change in trends or significance
...
Interaction is significant, improves fit, does not change parsimony
...
Same trends, but makes orchard trend slightly more significant
...
Higher mean temp is worse for provisioning in the early season relative to late season.
...
Interaction is not significant, does not improve fit, does not change parsimony
