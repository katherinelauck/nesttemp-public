...
m <- read_rds("data/morph.rds")
c <- read_rds("data/cort_with_id.rds")
t <- read_rds("data/temp.rds")
h <- read_rds("data/humidity.rds")
g <- read_rds("data/growth.rds")
s <- read_rds("data/survival.rds")
nc <- nestbox_drive_get("nest_checks")

### boxes monitored for survival

nc %>% mutate(year = year(nc$Date)) %>% group_by(year,Nestbox) %>% summarize(n()) %>% nrow()

### individuals banded

m %>% filter(`Band Number` != "NA") %>% nrow()

### box-days of temperature

t %>% group_by(date,box) %>% summarize(n()) %>% nrow()

### site-days of temperature

h %>% group_by(date,site) %>% summarize(n()) %>% nrow()

### number of measured nestlings

m %>% filter(Age == "L") %>% nrow()

### number of blood samples

c %>% pivot_longer(cols = c(s1,s2),names_to = "sample",values_to = "mean_cort") %>%
  filter(!is.na(mean_cort)) %>% nrow()

c %>% pivot_longer(cols = c(s1,s2),names_to = "sample",values_to = "mean_cort") %>%
  filter(!is.na(mean_cort)) %>% group_by(ind_id) %>% summarize(n()) %>% nrow()

## pivot tables for april scwa presentation

### number of attempts intensively monitored for growth by habitat and year
...
g %>% select(attempt_id,site,habitat,year) %>% distinct() %>% group_by(habitat,site,year) %>% summarize(n = n()) %>%
  pivot_wider(names_from = c(habitat,site), values_from = n) %>%
  ungroup() %>%
  gt() %>% tab_spanner_delim(delim = "_") %>%
  data_color(columns = -year,fn = scales::col_numeric(
    palette = viridis::viridis(18),
    domain = c(1,17)
  )) %>%
  cols_add(Total = c(7+6+5+9+5+12+11+17+4+5+5,
                   10+4+4+8+11+17+9+10+7+9+7,
                   4+7+4+6+6+7+4+1+4+5+5)) %>%
  grand_summary_rows(columns = -year,
                     fns = fns_labels) %>%
  gtsave("figures/numboxesbysite.png")
